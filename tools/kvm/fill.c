...
void usage(const char *s, int terminate) {
  ...
  printf("\t shmem args := [pci:]<addr>:<size>[:handle=<shm handle>][:create]]\n");
  printf("\t\t addr : isn't used, but here for compatibility with kvm tool\n");
  printf("\t\t size : size of shared memory segment in bytes\n");
  printf("\t\t shm handle: string to use for shm_open()\n");
  printf("\t\t create : should this program create and initialize shared memory segment\n");
  printf("\t -d : bump debug level\n");
  printf("\t -v : bump verbose level\n");
  printf("\t -w : don't exit immediately. Used to keep shmem segments alive for others\n");
...
int main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc,argv,"D:dvws:f:")) != -1) {
    ...
    case 'w':
      wait++;
      break;
    case 's': // update to accept 0x0dae
...
#if 0
  if (debug)
    printf("Sizeof: size_t(%d),off_t(%d),__u64(%d) unsigned long long(%d)\n",
	   sizeof(size_t),sizeof(off_t),sizeof(__u64),
	   sizeof(unsigned long long));
 #endif 
  host_mem = dae_shmem(key,buf_size,si->create,verbose);
  if (host_mem == NULL) {
    fflush(stderr);
    exit(1);
  }
  ...
  if (fname) {
    ...
    rt = fstat(fd,&statbuf);
    if ((unsigned long long)statbuf.st_size > (unsigned long long) buf_size) {
      printf(
	     "File, %s is too large. We allocated %ld MB but the file is %ld MB.\n",
	     fname,buf_size/1024/1024,statbuf.st_size/1024/1024
	     );
      exit(1);
    }
    mem_fd = mmap(0,statbuf.st_size,PROT_READ,MAP_PRIVATE,fd,0);
    if (mem_fd == NULL) {
