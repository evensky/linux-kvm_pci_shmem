...
int pci_shmem__register_mem(struct shmem_info *si)
{
  if (shmem_region == NULL)
    ...
    fprintf(stderr,"pci-shmem: only supporting a single region. ignoring request.\n");
    free(si);
  }
  return 0;
}
...
int
pci_shmem__init(struct kvm *kvm)
{
        u8 dev, line, pin;
        char *mem;
	int verbose = 0;
	if (irq__register_device(PCI_DEVICE_ID_PCI_SHMEM, &dev, &pin, &line) < 0)
	  return 0;
	// ignore irq stuff, just want bus info.
	//pci_shmem_pci_device.irq_pin		= pin;
	//pci_shmem_pci_device.irq_line	        = line;
	if (shmem_region == 0) {
	  if (verbose == 1)
	    fprintf(stderr,"pci_shmem_init: memory region not registered\n");
	  return 0;
	}
	pci_shmem_pci_device.bar[0] = shmem_region->phys_addr | PCI_BASE_ADDRESS_SPACE_MEMORY;
	pci_shmem_pci_device.bar_size[0] = shmem_region->size;
	pci__register(&pci_shmem_pci_device, dev);

	mem = dae_shmem(shmem_region->handle,shmem_region->size,shmem_region->create,verbose);
	if (mem == NULL)
	  return 0;
